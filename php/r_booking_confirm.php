...
if (!$referer == "localhost/php/r_booking.php") {
...
<div class="wrap">
  ...
  <div class="mr">
    <?php if($menbers["User"]["role"]!=3){
      echo $menbers_name=htmlspecialchars($menbers['User']['name'],ENT_QUOTES,'UTF-8')."様<br>予約はまだ完了していません。" ; }
      else{ echo "予約を受付できません。" ;} ?>
  </div>  
  <?php if($menbers["User"]["role"]!=3): ?>
    <div id="res_conf">
      <form action="r_booking_complete.php" method="post">
        <table>
          ...
          <input type="hidden" name="check_in" value="<?php echo $date1; ?>">
            ...
            <tr>
              <th>ご要望：</th><td><textarea id="textbox" rows="8" name="requests" value=""></textarea></td>
            </tr>
        </table>
        <input type="submit" class="edit_submit" value="予 約" id="btn" onClick="if(!confirm('本当に予約しますか？')) return false;">
      </form>
      <form action="r_booking.php" method="post">
        <input type="hidden" name="check_in" value="<?php echo $date1; ?>">
        <input type="hidden" name="check_out" value="<?php echo $date2; ?>">
        <input type="submit" class="edit_submit" value="戻る" id="btn"onClick="if(!confirm('前のページへ戻りますか？')) return false;">
      </form>
    </div>
  <?php else:?>
  <div class="mr">
